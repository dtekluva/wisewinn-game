...
interface SelectedScoresSlice {
  ...
  maximumMatchSelections: number;
  maximumScoreSelections: number;
  ...
  deleteMatch: (fixtureId: string) => void;
}
...
> = set => ({
  selectedScores: [],
  maximumMatchSelections: 3,
  maximumScoreSelections: 1,
  ...
  addNewScore: (newMatch, scoresType) =>
    set(state => {
      ...
      if (!hasMatchBeenAdded) {
        if (state.selectedScores.length >= state.maximumMatchSelections) {
          return { selectedScores: [...state.selectedScores.slice(1), newMatch] };
        } else {
      ...
      const totalScores =
        previouslyAddedMatch.customScores.length + previouslyAddedMatch.scores.length;

      let updatedMatch: SelectedScoreMatch;

      if (scoresType == 'scores') {
        previouslyAddedMatch.customScores = [];
        updatedMatch = {
          ...previouslyAddedMatch,
          scores: hasScoreBeenAdded
            ...
            totalScores >= state.maximumScoreSelections
            ? [...previouslyAddedMatch.scores.slice(1), newScore]
            : [...previouslyAddedMatch.scores, newScore],
        };
      } else if (scoresType == 'customScores') {
        previouslyAddedMatch.scores = [];
        updatedMatch = {
          ...previouslyAddedMatch,
          customScores: hasScoreBeenAdded
            ? // replace (refresh) score content if it has been added before
              previouslyAddedMatch[scoresType].map(s =>
                s.scoreText === newScore.scoreText ? newScore : s,
              )
            : // add it if it has not beeen added previously
            totalScores >= state.maximumScoreSelections
            ? [...previouslyAddedMatch.customScores.slice(1), newScore]
            : [...previouslyAddedMatch.customScores, newScore],
        };
      }
  ...
  deleteMatch: fixtureId =>
    set(state => ({
      selectedScores: state.selectedScores.filter(match => match.matchId != fixtureId),
    })),
});

interface SoccerCashSubmission {
  succeeded: boolean;
  message: string;
  game_play_id: string;
...
interface SoccerCashSuccessObject {
  game_id: string;
  game_status: string;
  game_type: string;
...
export const useAllInSoccerCashStore = create<
  SelectedScoresSlice & SubmissionsSlice & SuccessSlice
>()(
  persist(
    (...a) => {
