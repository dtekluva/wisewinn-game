...
export const TabSection = () => {
  ...
  const games = () => {
    const allGames = [
      ...(salaryForLifeGameWonHistory?.jackpot_winners || []),
      ...(instantCashOutGameWonHistory?.jackpot_winners || []),
      ...(wyseCashGameWonHistory?.jackpot_winners || []),
    ];
    ...
    } else if (pathname === DASHBOARDROUTE) {
      return allGames;
    }
  };
  ...
  return (
    <section className="salary_for_life  mt-[30px]">
      <h1 className="winners_table_view my-2 text-white">Winners</h1>
      ...
      <Tab.Group selectedIndex={selectedIndex} onChange={setSelectedIndex}>
        <Tab.List className="flex items-center space-x-1 overflow-auto rounded-[10px] bg-[#15171D] p-1">
          <Tab
            className={({ selected }) =>
              clsx(
                'mr-4 w-full whitespace-nowrap border-b-[3px] p-3 text-sm font-medium  leading-5 ring-white ring-opacity-60 md:w-[25%] ',
                selected ? 'border-[#B500FF] text-[#B500FF]' : 'border-transparent text-white',
                [DASHBOARDROUTE, SALARYFORLIFEROUTE, CAMPAIGNSALARYFORLIFEROUTE].includes(pathname)
                  ? 'block'
                  : 'hidden',
              )
            }
          >
            Salary for Life Winners
          </Tab>
          <Tab
            className={({ selected }) =>
              clsx(
                'mx-4 w-full whitespace-nowrap border-b-[3px] p-3 text-sm font-medium  leading-5 ring-white ring-opacity-60 md:w-[25%] ',
                selected ? 'border-[#B500FF] text-[#B500FF]' : 'border-transparent text-white',
                [DASHBOARDROUTE, INSTANTCASHOUTROUTE, CAMPAIGNINSTANTCASHOUTROUTE].includes(
                  pathname,
                )
                  ? 'block'
                  : 'hidden',
          ...
          <Tab
            className={({ selected }) =>
              clsx(
                'mx-4 w-full whitespace-nowrap border-b-[3px] p-3 text-sm font-medium  leading-5 ring-white ring-opacity-60 md:w-[25%] ',
                selected ? 'border-[#B500FF] text-[#B500FF]' : 'border-transparent text-white',
                [DASHBOARDROUTE, WHYSECASHROUTE, CAMPAIGNWHYSECASHROUTE].includes(pathname)
                  ? 'block'
                  : 'hidden',
              )
            }
          ...
          <Tab.Panels>
            <Tab.Panel className="rounded-[10px] p-[0px] xl:py-[10px]">
              ...
              <DashTable
                ...
                playGameRoute={SALARYFORLIFEROUTE}
            ...
            <Tab.Panel className="rounded-[10px] p-[0px] xl:py-[10px]">
              ...
              <DashTable
                ...
                playGameRoute={INSTANTCASHOUTROUTE}
                emptyNotice={'No Game Won History yet'}
                emptyNoticeSubheading={
                  'You are yet to have any Game won by any user. Details of your Game won by users would be displayed here.'
                }
