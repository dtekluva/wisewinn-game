...
import { usePhoneLogin } from '@/features/auth';
import { useNotificationModalControl } from '@/hooks';
import { formatAxiosErrorMessage } from '@/utils';

export const PhoneNumberLogin: React.FunctionComponent = () => {
  ...
  const { mutate: postLogIn, isLoading } = usePhoneLogin();
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="w-full">
        <div className="mt-5">
          ...
          {errors.phone && <InputError text="This field is required" />}
        </div>
        <div className="my-5">
          <div className="flex w-full rounded-md !bg-[#15171D] p-0 pr-3">
            <InputField
              className="!bg-[#15171D] text-white"
              id="password"
        ...
        <button
          type="submit"
          className="bg_red_gradient md:basefont-semibold my-2 w-full rounded py-2 text-center text-base font-semibold text-white disabled:text-gray-400 md:py-4"
          disabled={isLoading}
        >
