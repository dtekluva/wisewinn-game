...
export const CounterDownMinutes = () => {
  ...
  const [randomNumberID, setRandomNumberID] = React.useState(randomId);
  ...
  return (
    <div className="relative my-2 flex w-full items-center justify-between gap-2  overflow-y-visible bg-[#15171D]  px-4 py-4 md:gap-6 md:px-8 lg:px-10">
      <p className="text-sm font-medium text-[#A6A6A6] lg:text-lg">
        <span>Game ID: </span>
        <span className="text-sm font-semibold text-[#fff] lg:text-xl">{randomNumberID}</span>
      </p>

      <p className="text-right text-sm font-medium text-[#A6A6A6] lg:text-lg">
        <span>
          Game <span className="hidden md:inline">batch</span> expires in 5 minutes:{' '}
        </span>
